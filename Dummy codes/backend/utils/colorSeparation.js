...
export async function analyzeColors(imageBuffer, imageSize = 300) {
  ...
  console.log(`Color distribution: Red=${colorCounts[0]}, Green=${colorCounts[1]}, Blue=${colorCounts[2]}, Black=${colorCounts[3]}`);
  ...
  console.log('Detected colors:', colors.map(c => `${c.name} (${c.hex})`).join(', '));
